...
def test_upload_receipt_and_process_success(mock_process_image, mock_normalize, mock_create_record):
    """レシートアップロード、OCR、正規化提案のフローテスト"""
    # 1. OCR結果のモック
    mock_raw_data = {
        "store_name": "ファミマ", "item_name": "牛乳パック", "price": 240.0, "purchase_date": date.today().isoformat() 
    }
    mock_process_image.return_value = [mock_raw_data] 

    # 2. 正規化結果のモック (既存アイテムへの名寄せを提案)
    mock_normalize.return_value = {
        "raw_item_name": "牛乳パック", "raw_store_name": "ファミマ", "raw_price": 240.0,
        "raw_purchase_date": date.today().isoformat(), # dateをisoformat文字列に変更
        "is_new_item": False, "suggested_item_id": 101, "suggested_item_name": "牛乳",
        "is_new_store": False, "suggested_store_id": 201, "suggested_store_name": "ファミリーマート"
    }
...
def test_confirm_and_register_record_success(mock_create_record):
    ...
    mock_record_in = RecordCreate(
        raw_item_name="牛乳パック", raw_store_name="ファミマ", raw_price=240.0,
        raw_purchase_date=date.today(), item_id=101, store_id=201,
        final_price=240.0, final_purchase_date=date.today()
    )
    mock_record_out = Record(
        id=500, user_id=MOCK_USER.id, price=240.0, purchase_date=date.today(),
        item_id=101, store_id=201, item_name="牛乳", store_name="ファミリーマート"
    )
    mock_create_record.return_value = mock_record_out

    response = client.post("/api/v1/receipts/confirm", json=mock_record_in.model_dump(mode='json', by_alias=True))
...
def test_update_item_owner_check_failure(mock_update_item):
    ...
    item_in = ItemCreate(normalized_name="コーラ 500ml")
...
def test_suggest_items_feature(mock_suggest_items):
    ...
    mock_suggest_items.return_value = [
        Item(id=10, user_id=MOCK_USER.id, normalized_name="サッポロ一番"),
        Item(id=11, user_id=MOCK_USER.id, normalized_name="サッポロポテト")
    ]
    ...
    data = response.json()
    assert len(data) == 2
    assert "サッポロ一番" in data[0]["normalized_name"]
...
def test_get_price_comparison_success(mock_comparison):
    ...
    mock_comparison.return_value = [
        {"item_name": "納豆", "store_name": "Aスーパー", "average_price": 98.0, "overall_average_price": 102.0},
        {"item_name": "納豆", "store_name": "Bコンビニ", "average_price": 110.0, "overall_average_price": 102.0},
    ]
...
def test_export_data_csv_export(mock_export_csv, mock_remove):
    ...
    TEST_CSV_CONTENT = "date,store_name,item_name,price\n2023-10-01,Aスーパー,牛乳,200\n"
    ...
    assert f'filename=purchase_history_{MOCK_USER.id}.csv' in response.headers["content-disposition"]
...
def test_create_store_success(mock_create_store):
    ...
    mock_create_store.return_value = Store(id=301, user_id=MOCK_USER.id, name="新規スーパー")
