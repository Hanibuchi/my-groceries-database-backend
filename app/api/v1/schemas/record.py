...
class OCRResult(BaseModel):
    ...
    # 1. OCRで読み取った生データ (必須)
    raw_item_name: str = Field(
        ..., max_length=255
    )  # OCRから読み取った商品名（正規化前）
    raw_store_name: str = Field(
        ..., max_length=255
    )  # OCRから読み取った店舗名（正規化前）
    raw_price: str = Field(..., max_length=255)  # OCRから読み取った価格
    raw_purchase_date: str = Field(
        default_factory=str(date.today)
    )  # OCRから読み取った日付

    # 2. 名寄せ候補や確認情報 (正規化の提案)
    is_new_item: bool = True  # 商品名が新規登録の必要があるか
    suggested_item_id: Optional[int] = None  # 名寄せされた既存のitem_id（あれば）
    suggested_item_name: Optional[str] = Field(
        ..., max_length=255
    )  # item_name (表示用)
